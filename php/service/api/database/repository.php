...
//            $this->connection->set_charset($charset);
    ...
    public function query($sqlQuery)
    {
        if (mysqli_query($this->connection, $sqlQuery)) {
            if ($this->logFilename) {
                writeToLog($this->logFilename, "Info|Write request to database|#" .
            ...
            $this->errorMessage = mysqli_error($this->connection);
            if ($this->logFilename) {
                writeToLog($this->logFilename, "Error|SQL query to database failed|#" .
    ...
    public function updateQuery($sqlQuery)
    {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $updated = $row[0];
            if ($this->logFilename) {
                writeToLog($this->logFilename, "Info|Update request from database|#" .
    ...
    public function getIntValue($sqlQuery)
    {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $selected = $row[0];
            if ($this->logFilename) {
                writeToLog($this->logFilename, "Info|Read data from database|#" .
